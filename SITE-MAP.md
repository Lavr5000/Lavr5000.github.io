...
- [index.html](index.html) - main landing page for public visitors: the two AI setup services (Claude Code on one workstation, Hermes Agent on laptop + phone), when they fit, how the setup goes, who runs it, and the construction tools as a secondary direction. The services section is a teaser — names, one sentence, "от 15 000 ₽" and a link to `/services/`; the full cards and the order dialog live on `/services/` only, and the hero cards link to `/services/#service-1` and `#service-2`. The `#services` anchor is kept because published posts point at `/#services`. Since 02.09.2026 the page also carries `#apps` ("06 · Бесплатно") — two free applications, Transkribator linking to [apps/transkribator/](apps/transkribator/) and Apartment Auditor linking straight to its RuStore listing; the contact section moved from kicker 06 to 07, ids unchanged. Styles in [assets/site2.css](assets/site2.css), behaviour in [assets/index2.app.js](assets/index2.app.js), hero terminal frames in [assets/hero-demo.js](assets/hero-demo.js).
- [services/index.html](services/index.html) - services page at the stable public URL `/services/`, the address publications link to. The only place carrying the full service cards, the price and the order dialog; self-canonical, no payment form. Since 15.08.2026 it covers **two directions in two sections**: `#services` (01, настройка AI — `#service-1`, `#service-2`) and `#documents` (02, работа с документами — `#service-3` извлечение данных из чертежей КЖ, `#service-4` комплект исполнительной документации). The document services are operator-run: the client sends files by message, the operator runs the pipeline and checks the result by hand, so each card prints its billable unit, its included volume and its boundary («КЖ / монолитные конструкции; АР, АС и прочие разделы не берём», «исполнительные схемы не изготавливаются»). `#process` keeps its id and now carries two four-step tracks, one per direction.
- [about.html](about.html) - public "about" page for visitors who want context about the project, author, or service positioning. Static page on [assets/site2.css](assets/site2.css); the AI-setup price lives on `/services/` only, and the page is Russian-only. The «Системы» section lists eight projects (P-01…P-08); Transkribator was inserted as P-02 on 02.09.2026 and the cards below it shifted by one, ids `#p-02`…`#p-08` with them.
- [apps/transkribator/](apps/transkribator/) - public page for the free Windows dictation app Transkribator: what it does, install steps (including the SmartScreen notice), requirements, an honest note on recognition quality, and the download button pointing at the GitHub release `releases/latest`. Static page on [assets/site2.css](assets/site2.css) + [assets/legal2.css](assets/legal2.css) + [assets/blog.css](assets/blog.css); no version or archive size in the copy (they change with every release), no form and no API call. Self-canonical (`/apps/transkribator/`), absolute asset paths so the deep URL resolves.
...
- [services.js](services.js) is the registry behind the cards: an immutable kebab-case `id` per service plus the one `title` used on the site, in the offer terms, in the payment link and on the receipt. Since prices stopped being uniform (15 000 ₽ / 15 000 ₽ / от 10 000 ₽ / 3 500 ₽) each record also carries `pricingModel` (`fixed` | `from`) and `priceLabel.ru` — the exact rendered string. Every order button carries `data-service-id`, the `data-order` display text and `data-price-label`; the modal reads the price off the button, so there is no hardcoded price left in [services/index.html](services/index.html) or [assets/index2.app.js](assets/index2.app.js). The file is not loaded by any page — it is the source of truth for the validator, the offer terms and the manual request register.
- **Whitespace canon for prices:** every price string uses U+0020 only, никаких NBSP и `&nbsp;` — otherwise the byte-for-byte comparison between registry, button and card is meaningless. The validator rejects a string that merely *renders* the same.
- `node scripts/validate_services.mjs` gates three halves: the registry schema (unique kebab-case ids, non-empty fields, no duplicate titles, name length within the 64-character receipt cap, `priceLabel` shape agreeing with `price` and `pricingModel`); every order button resolving to a known id whose `title` matches `data-order` and whose `priceLabel` matches `data-price-label` byte for byte; and the visible `.panel-tag.price` of the card the button sits in. Buttons are found by scanning all `<button>` tags, so a CTA added later is covered — a button outside a card (the final «Оставить заявку») is excluded from the card-price check by design. It is a local gate, not a CI workflow — `validate-products.yml` covers [products.js](products.js) only.
- `python tests/preview_services_check.py [base_url] [--shots DIR]` is the preview gate: it drives a real Chromium against a local `http.server` and proves, at 1440×1000 / 768×1024 / 390×844, zero console errors, no horizontal scroll, panel disclosure on keyboard focus (`:focus-within`), `prefers-reduced-motion`, and — enumerating every `[data-order]` trigger by selector, never by count — that each modal and each generated `mailto:` carries that service's own title and its own price label.
